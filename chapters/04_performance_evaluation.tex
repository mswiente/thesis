...
Related work can be categorised in two different topics, performance measuring and evaluation and performance modelling. 
...
\begin{itemize}
	\item Two different approaches: Performance Measuring and Performance Prediction
	\item Performance Measuring 
	\begin{itemize}
		\item can only be done with an existing system
	\end{itemize}
	\item Performance Prediction
	\begin{itemize}
		\item allows to predict the performance of a system in an early stage of development before the system is available
		\item uses Performance Modelling to build a model of the system, evaluation is done with this model
		\item common approaches use queuing networks to model a messaging system
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item instead of using a model a prototype has been implemented, because
	\item no suitable models exist (? - needs to be checked, there should be many models for messaging systems...)
	\item it's difficult to build a model since every relevant aspect needs to be modelled (data transfer, data marshalling, database, transactions)
	\item prototype is a compromise between using a (mathematical or theoretical) model and a real application. 
	\item By using state-of the art technologies for the implementation, the relevant aspects for comparing the different processing types come for ``free''.
\end{itemize}
...
\begin{itemize}
	\item related research is concerned with the performance of messaging middleware such as JMS Servers
	\item In this research an end-to-end performance evaluation of a batch and messaging prototype implementation has been conducted instead
\end{itemize}
