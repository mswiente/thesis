...
\begin{table}
	...
	\caption{Used frameworks and technologies}
...
\begin{itemize}
	\item The comparison should only include the processing (integration ?) style of the prototypes
	\item All other aspects should be the same, for example business functionality, data access, datamodel
	\item A common architecture is used to ensure comparability between the two prototypes
\end{itemize}

% subsection common_architecture (end)

\subsubsection{Datamodel}
The prototypes use a common data model.
...
\begin{itemize}
	\item Business functionality (Mediation, Rating) is implemented by Business Services used by both prototypes
	\item Interface DataProcessor 
	\item MediationProcessor, implements Mediation functionality
	\item RatingProcessor, implements Rating functionality
	\item Describe functionality in detail using a sequence diagram (?)
\end{itemize}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\columnwidth]{ch4_business_services}
...
\begin{itemize}
	\item The prototypes use a common data access layer
	\item Uses OR Mapper OpenJPA (JPA 2.0)
	\item Business Entities are generated from Database Model
	\item Data Access is implemented using the DAO pattern
\end{itemize}

\subsubsection{Technical Integration}

\begin{itemize}
	\item Only difference is the processing type, implemented by the Integration Layer of the respective prototype
	\item Batch frame
	\item Messaging middleware
\end{itemize}
